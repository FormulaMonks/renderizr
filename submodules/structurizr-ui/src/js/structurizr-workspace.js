...
structurizr.Workspace = class Workspace {
    ...
    hasDocumentation() {
        ...
        var result = false;
        this.#workspace.model.softwareSystems.forEach(
            (softwareSystem) => {
        ...
        if (model.customElements) {
            ...
            for (i = 0; i < model.customElements.length; i++) {
                ...
                this.#registerElement(
                    customElement,
                    structurizr.constants.CUSTOM_ELEMENT_TYPE,
                );
            }
        } else {
            ...
            for (i = 0; i < model.softwareSystems.length; i++) {
                ...
                if (softwareSystem.containers) {
                    ...
                    for (var j = 0; j < softwareSystem.containers.length; j++) {
                        ...
                        if (container.components) {
                            this.#sortArrayByNameAscending(
                                container.components,
                            );
                            for (
                                var k = 0;
                                k < container.components.length;
                                k++
                            ) {
    ...
    #registerDeploymentNode(deploymentNode, parent) {
        ...
        if (deploymentNode.environment === undefined) {
            deploymentNode.environment =
                structurizr.constants.DEFAULT_DEPLOYMENT_ENVIRONMENT_NAME;
        }
        ...
        if (deploymentNode.softwareSystemInstances) {
            for (
                var i = 0;
                i < deploymentNode.softwareSystemInstances.length;
                i++
            ) {
                const softwareSystemInstance =
                    deploymentNode.softwareSystemInstances[i];
                const softwareSystem = this.findElementById(
                ...
                if (softwareSystemInstance.description === undefined) {
                    softwareSystemInstance.description =
                ...
                if (softwareSystemInstance.environment === undefined) {
                    softwareSystemInstance.environment =
                        deploymentNode.environment;
                }
        ...
        if (deploymentNode.containerInstances) {
            for (var i = 0; i < deploymentNode.containerInstances.length; i++) {
                ...
                const container = this.findElementById(
        ...
        if (deploymentNode.infrastructureNodes) {
            for (
                var i = 0;
                i < deploymentNode.infrastructureNodes.length;
                i++
            ) {
                const infrastructureNode =
                    deploymentNode.infrastructureNodes[i];
    ...
    getTags() {
        ...
        Object.keys(this.#relationshipsById).forEach((id) => {
            const relationship = this.#relationshipsById[id];
            if (relationship.tags) {
                relationship.tags.split(",").forEach((tag) => {
                    tag = structurizr.util.trim(tag);
    ...
    getAllPropertiesForElement(element) {
        ...
        if (
            ...
            if (softwareSystem && softwareSystem.properties) {
                Object.keys(softwareSystem.properties).forEach((key) => {
                    if (!properties[key]) {
            ...
            if (container && container.properties) {
                Object.keys(container.properties).forEach((key) => {
                    if (!properties[key]) {
    ...
    getAllTagsForRelationship(relationship) {
        ...
        while (linkedRelationshipId !== undefined) {
            ...
            var linkedRelationship =
                this.findRelationshipById(linkedRelationshipId);
    ...
    getAllPropertiesForRelationship(relationship) {
        ...
        while (linkedRelationshipId !== undefined) {
            ...
            if (linkedRelationship && linkedRelationship.properties) {
                Object.keys(linkedRelationship.properties).forEach(
                    (key) => {
                        if (!properties[key]) {
                            properties[key] =
                                linkedRelationship.properties[key];
                        }
                    },
                );
            }
    ...
    #registerRelationship(relationship) {
        ...
        while (
            (relationship.url === undefined ||
                relationship.url.trim().length === 0) &&
    ...
    #initViews() {
        ...
        this.#workspace.views.customViews.forEach((view) => {
        ...
        this.#workspace.views.systemLandscapeViews.forEach((view) => {
        ...
        this.#workspace.views.systemContextViews.forEach((view) => {
        ...
        this.#workspace.views.containerViews.forEach((view) => {
        ...
        this.#workspace.views.componentViews.forEach((view) => {
        ...
        this.#workspace.views.dynamicViews.forEach((view) => {
        ...
        this.#workspace.views.deploymentViews.forEach((view) => {
            if (
                view.environment === undefined ||
                view.environment.trim().length === 0
            ) {
                view.environment =
                    structurizr.constants.DEFAULT_DEPLOYMENT_ENVIRONMENT_NAME;
            }
        ...
        this.#workspace.views.filteredViews.forEach((view) => {
        ...
        this.#workspace.views.imageViews.forEach((view) => {
        ...
        if (this.#workspace.views.configuration.metadataSymbols === undefined) {
            this.#workspace.views.configuration.metadataSymbols =
                "SquareBrackets";
        }
    ...
    #registerView(view) {
        ...
        if (view.type !== structurizr.constants.IMAGE_VIEW_TYPE) {
            ...
            if (view.type !== structurizr.constants.DYNAMIC_VIEW_TYPE) {
                ...
                view.animations.forEach((animationStep) => {
    ...
    findSystemContextViewsForSoftwareSystem(softwareSystemId) {
        ...
        for (var i = 0; i < this.#views.length; i++) {
            ...
            if (
                view.type === structurizr.constants.SYSTEM_CONTEXT_VIEW_TYPE &&
                view.softwareSystemId === softwareSystemId
            ) {
                views.push(view);
                ...
                if (
                    ...
                    baseView.softwareSystemId === softwareSystemId
                ) {
                    views.push(view);
                }
            }
    ...
    findContainerViewsForSoftwareSystem(softwareSystemId) {
        ...
        for (var i = 0; i < this.#views.length; i++) {
            ...
            if (
                ...
                if (
                    baseView.type ===
                        structurizr.constants.CONTAINER_VIEW_TYPE &&
                    baseView.softwareSystemId === softwareSystemId
                ) {
                    views.push(view);
                }
    ...
    findComponentViewsForContainer(containerId) {
        ...
        for (var i = 0; i < this.#views.length; i++) {
            ...
            if (
                ...
                if (
                    baseView.type ===
                        structurizr.constants.COMPONENT_VIEW_TYPE &&
                    baseView.containerId === containerId
                ) {
                    views.push(view);
                }
            }
    ...
    #sortViews() {
            ...
            if (filters.indexOf(view.key) === -1) {
                ...
                if (obj.element) {
                    ...
                    if (
                        ...
                        obj.softwareSystem = this.findElementById(
                            obj.element.parentId,
                        );
                    } else if (
                        obj.element.type ===
        ...
        try {
            if (
                this.#workspace.views.configuration.viewSortOrder === "Key" ||
                (this.#workspace.views.configuration.properties &&
                    this.#workspace.views.configuration.properties[
                        "structurizr.sort"
                    ] === "key")
            ) {
                listOfViews.sort((a, b) => a.key.localeCompare(b.key));
            } else if (
                this.#workspace.views.configuration.viewSortOrder === "Type" ||
                (this.#workspace.views.configuration.properties &&
                    this.#workspace.views.configuration.properties[
                        "structurizr.sort"
                    ] === "type")
            ) {
                listOfViews.sort((a, b) => (a.viewTypeOrder + a.scope + a.key).localeCompare(
                ...
                listOfViews.sort((a, b) => a.view.order - b.view.order);
            } else {
                listOfViews.sort((a, b) => {
                    if (
                        a.softwareSystem === undefined &&
                        b.softwareSystem !== undefined
                    ) {
                        return -1;
                    }
                    ...
                    if (
                        a.softwareSystem.name.localeCompare(
                        ...
                        return a.softwareSystem.name.localeCompare(
                            b.softwareSystem.name,
                        );
                    }
                });
        ...
        this.#views = listOfViews.map((v) => v.view);
    }
    ...
    hasStyles() {
        return (
            this.#workspace.views.configuration.styles.elements.length > 0 ||
            this.#workspace.views.configuration.styles.relationships.length > 0
        );
    }
    ...
    findElementStyleByTag(tag) {
        ...
        this.#workspace.views.configuration.styles.elements.forEach(
            (elementStyle) => {
    ...
    #getTerminology(key, defaultValue) {
        if (
            this.#workspace.views.configuration.terminology.hasOwnProperty(key)
        ) {
    ...
    getTerminologyFor(item) {
        if (item.type === "Person") {
            ...
            item.type === "SoftwareSystem" ||
            item.type === "SoftwareSystemInstance"
        ) {
            return this.#getTerminology("softwareSystem", "Software System");
        } else if (
            item.type === "Container" ||
            item.type === "ContainerInstance"
        ) {
    ...
    #sortProperties(modelItem) {
        if (modelItem.properties) {
            ...
            Object.keys(modelItem.properties)
                .sort()
                .forEach((key) => {
