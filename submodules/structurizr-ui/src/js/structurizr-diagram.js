structurizr.ui.Diagram = function (
    id,
    diagramIsEditable,
    constructionCompleteCallback,
) {
    const self = this;
    ...
    parentElement.append(
        '<div id="' +
            viewportId +
            '" class="structurizrDiagramViewport"><div id="' +
            canvasId +
            '" class=structurizrDiagramCanvas"></div></div>',
    );
    ...
    const paper = new joint.dia.Paper({
        ...
        linkView: joint.dia.LinkView.extend({
            pointerclick: (evt, x, y) => {
                if (editable) {
                    if (evt.altKey) {
                        // do nothing
                    } else {
                        if (
                            V(evt.target).hasClass("connection") ||
    ...
    function getImagesToPreload() {
        ...
        tags.push("Group");
        structurizr.workspace.views.configuration.styles.elements.forEach(
            (elementStyle) => {
        ...
        structurizr.ui.themes.forEach((theme) => {
            theme.elements.forEach((elementStyle) => {
                if (elementStyle.tag.indexOf("Group:") > -1) {
                    tags.push(elementStyle.tag);
                }
            });
        ...
        structurizr.workspace.views.imageViews.forEach((view) => {
    ...
    function preloadImages() {
        if (imageMetadata === undefined) {
            ...
            getImagesToPreload().forEach((url) => {
                ...
                image.addEventListener("error", function (error) {
                    ...
                    console.log(
                        "There was an error loading the image " +
                            im.src +
                            " - please check that the image exists, and that the Access-Control-Allow-Origin header is set to allow cross-origin requests.",
                    );
                    im.loaded = true;
                    im.error = true;
        ...
        if (
            ...
            imageMetadata.filter((im) => im.loaded === false).length > 0 &&
            imagePreloadAttempts < 50
        ) {
    ...
    this.isRendered = () => diagramRendered;
    ...
    function shapePerimeterConnectionPoint(linkView, view, magnet, reference) {
        ...
        if (linkView.model.get("router") === undefined) {
            ...
            if (
                ...
                var radians = Math.atan2(
                    centrePointY - reference.y,
                    reference.x - centrePointX,
                );
        ...
        if (magnet) {
            spot = V(magnet).findIntersection(
                reference,
                linkView.paper.viewport,
            );
            if (!spot) {
    ...
    this.onViewChanged = (f) => {
        viewChangedEventHandler = f;
    };

    this.changeView = (key, callback) => {
    ...
    function renderView(callback) {
        ...
        paper.setInteractivity((cellView) => {
            ...
            (elementStyle) => {
                if (elementStyle.tag === "Diagram:Title") {
                    ...
                    if (elementStyle.fontSize) {
                        elementStyleForDiagramTitle.fontSize =
        ...
        structurizr.workspace.views.configuration.styles.elements.forEach(
            (elementStyle) => {
                if (elementStyle.tag === "Diagram:Description") {
                    if (elementStyle.color) {
                        elementStyleForDiagramDescription.color =
                            elementStyle.color;
                    }
                    if (elementStyle.fontSize) {
                        elementStyleForDiagramDescription.fontSize =
                            elementStyle.fontSize;
                    }
                }
            },
        ...
        structurizr.workspace.views.configuration.styles.elements.forEach(
            (elementStyle) => {
                if (elementStyle.tag === "Diagram:Metadata") {
                    if (elementStyle.color) {
                        elementStyleForDiagramMetadata.color =
                            elementStyle.color;
                    }
                    if (elementStyle.fontSize) {
                        elementStyleForDiagramMetadata.fontSize =
                            elementStyle.fontSize;
                    }
                }
            },
        ...
        if (view.type === structurizr.constants.IMAGE_VIEW_TYPE) {
            ...
            if (imageMetadata.error) {
                ...
                imageMetadata.contentType =
                    structurizr.constants.CONTENT_TYPE_IMAGE_PNG;
            ...
            if (
                imageMetadata.contentType ===
                structurizr.constants.CONTENT_TYPE_IMAGE_SVG
            ) {
                ...
                if (imageMetadata.ratio >= 1 && imageWidth < minimumWidth) {
                    ...
                    imageMetadata.ratio < 1 &&
                    imageHeight < minimumHeight
                ) {
                    // portrait image
        ...
        if (view.elements) {
            for (var i = 0; i < view.elements.length; i++) {
                var positionX;
                var positionY;
                var element = structurizr.workspace.findElementById(
                    view.elements[i].id,
                );

                if (
                    !includeElementOnDiagram(element) ||
                    element.type ===
                        structurizr.constants.DEPLOYMENT_NODE_ELEMENT_TYPE
                ) {
                    continue;
                }
                ...
                if (elementStyle.shape === "Cylinder") {
                    ...
                    box = createMobileDevicePortrait(
                        view,
                        element,
                        elementStyle,
                        positionX,
                    ...
                    box = createMobileDeviceLandscape(
                        view,
                        element,
                        elementStyle,
                        positionX,
                ...
                box.on("change:position", (cell, newPosition, opt) => {
                    if (opt.translateBy === undefined) {
                        ...
                        if (
                            translatedByCell.attributes.type ===
                ...
                if (
                    ...
                    (view.type === structurizr.constants.DYNAMIC_VIEW_TYPE &&
                        view.elementId === undefined)
                ) {
                    var includeEnterpriseBoundary =
                    ...
                    if (
                        ...
                        includeEnterpriseBoundary =
                            view.properties[
                                "structurizr.enterpriseBoundary"
                            ] === "true";
                    }
                    if (
                        element.location &&
                        ...
                        if (!enterpriseBoundary) {
                            var enterprise =
                                structurizr.workspace.model.enterprise;
                            var boundaryName =
                                enterprise && enterprise.name
                                    ? enterprise.name
                                    : "Enterprise";

                            enterpriseBoundary = createBoundary(
                                boundaryName,
                                structurizr.ui.getMetadataForElement({
                                    type: "Enterprise",
                                }),
                                "Enterprise",
                            );
                ...
                if (
                    view.type ===
                        structurizr.constants.SYSTEM_CONTEXT_VIEW_TYPE &&
                    element.id === view.softwareSystemId
                ) {
                ...
                if (
                    ...
                    element.type ===
                        structurizr.constants.CONTAINER_ELEMENT_TYPE
                ) {
                ...
                if (
                    ...
                    element.type ===
                        structurizr.constants.COMPONENT_ELEMENT_TYPE
                ) {
                    // component on a component diagram - add a boundary to represent the parent container
                    const includeParentBoundary =
                        view.properties &&
                        view.properties[
                            "structurizr.softwareSystemBoundaries"
                        ] === "true";
                    addElementToBoundary(element, box, includeParentBoundary);
                }
                ...
                if (
                    ...
                    var scopedElement = structurizr.workspace.findElementById(
                        view.elementId,
                    );
                    ...
                    if (
                        ...
                        addElementToBoundary(
                            element,
                            box,
                            includeParentBoundary,
                        );
                    }
                }
            ...
            if (view.elements) {
                view.elements.forEach((elementView) => {
                    ...
                    if (
                        ...
                        if (
                            element.softwareSystemInstances &&
                            element.softwareSystemInstances.length > 0
                        ) {
                            element.softwareSystemInstances.forEach(
                                (softwareSystemInstance) => {
                                    // find the software system on the diagram
                                    var softwareSystemBox =
                                        cellsByElementId[
                                            softwareSystemInstance.id
                                        ];
                                    if (softwareSystemBox !== undefined) {
                                        if (
                                            softwareSystemInstance.group !==
                                            undefined
                                        ) {
                                            const rootGroup = findRootGroup(
                                                softwareSystemInstance.group,
                                                softwareSystemInstance.parentId,
                                            );
                                            if (rootGroup) {
                                                deploymentNodeCell.embed(
                                                    rootGroup,
                                                );
                                            } else {
                                                deploymentNodeCell.embed(
                        ...
                        if (
                            ...
                            element.containerInstances.length > 0
                        ) {
                            element.containerInstances.forEach(
                                (containerInstance) => {
                                    // find the container on the diagram
                                    var containerBox =
                                        cellsByElementId[containerInstance.id];
                                    if (containerBox !== undefined) {
                                        if (
                                            containerInstance.group !==
                                            undefined
                                        ) {
                                            const rootGroup = findRootGroup(
                                                containerInstance.group,
                                                containerInstance.parentId,
                                            );
                                            if (rootGroup) {
                                                deploymentNodeCell.embed(
                                                    rootGroup,
                        ...
                        if (
                            element.infrastructureNodes &&
                            element.infrastructureNodes.length > 0
                        ) {
                            element.infrastructureNodes.forEach(
                                (infrastructureNode) => {
                                    // find the infrastructure node on the diagram
                                    var infrastructureBox =
                                        cellsByElementId[infrastructureNode.id];
                                    if (infrastructureBox !== undefined) {
                                        if (
                                            infrastructureNode.group !==
                                            undefined
                                        ) {
                                            const rootGroup = findRootGroup(
                                                infrastructureNode.group,
                                                infrastructureNode.parentId,
                                            );
                                            if (rootGroup) {
                                                deploymentNodeCell.embed(
                                                    rootGroup,
                ...
                view.elements.forEach((elementView) => {
                    ...
                    if (
                        element.type ===
                            structurizr.constants
                                .SOFTWARE_SYSTEM_INSTANCE_ELEMENT_TYPE ||
                        element.type ===
                            structurizr.constants
                                .CONTAINER_INSTANCE_ELEMENT_TYPE ||
                        element.type ===
                            structurizr.constants
                                .INFRASTRUCTURE_NODE_ELEMENT_TYPE
                    ) {
                        ...
                        while (parentId) {
                            ...
                            var index =
                                unusedDeploymentNodeCells.indexOf(parentCell);
                            if (index > -1) {
                ...
                unusedDeploymentNodeCells.forEach(
                    (deploymentNodeCell) => {
                        deploymentNodeCell.remove();
                        delete cellsByElementId[
                            deploymentNodeCell.elementInView.id
                        ];
                        delete mapOfIdToBox[
                            deploymentNodeCell.elementInView.id
                        ];
    ...
    function repositionAllParentCells() {
        ...
        currentView.elements.forEach((elementView) => {
            ...
            if (
                ...
                element.type ===
                    structurizr.constants.SOFTWARE_SYSTEM_ELEMENT_TYPE ||
                ...
                element.type ===
                    structurizr.constants
                        .SOFTWARE_SYSTEM_INSTANCE_ELEMENT_TYPE ||
                element.type ===
                    structurizr.constants.CONTAINER_INSTANCE_ELEMENT_TYPE ||
                element.type ===
                    structurizr.constants.INFRASTRUCTURE_NODE_ELEMENT_TYPE
            ) {
                repositionParentCells(cell);
            }
        });
    ...
    function includeGroup(element, view) {
        ...
        if (
            view.properties &&
            view.properties["structurizr.groups"] === "false"
        ) {
            return false;
        }
        ...
        if (
            ...
            if (view.elementId === undefined) {
                renderGroupForElement =
                    element.type ===
                        structurizr.constants.PERSON_ELEMENT_TYPE ||
                    element.type ===
                        structurizr.constants.SOFTWARE_SYSTEM_ELEMENT_TYPE ||
                    element.type === structurizr.constants.CUSTOM_ELEMENT_TYPE;
            } else {
    ...
    function addElementToBoundary(element, cell, includeParentBoundary) {
        ...
        if (boundary === undefined) {
            var boundaryElement = structurizr.workspace.findElementById(
                element.parentId,
            );
            if (boundaryElement) {
                boundary = createBoundary(
                    boundaryElement.name,
                    structurizr.ui.getMetadataForElement(boundaryElement),
                    undefined,
                    boundaryElement,
                );
            ...
            if (includeParentBoundary && boundaryElement.parentId) {
                var parentBoundary =
                    boundariesByElementId[boundaryElement.parentId];
                var parentBoundary =
                    boundariesByElementId[boundaryElement.parentId];
                if (parentBoundary === undefined) {
                    var parentBoundaryElement =
                        structurizr.workspace.findElementById(
                            boundaryElement.parentId,
                        );
                    if (parentBoundaryElement) {
                        parentBoundary = createBoundary(
                            parentBoundaryElement.name,
                            structurizr.ui.getMetadataForElement(
                                parentBoundaryElement,
                            ),
                            undefined,
                            parentBoundaryElement,
                        );
    ...
    function findRootGroup(name, scope) {
        if (useNestedGroups()) {
            ...
            if (name.indexOf(separator) > -1) {
                return findGroup(
                    name.substring(0, name.indexOf(separator)),
                    scope,
                );
            } else {
    ...
    function getGroupSeparator() {
        return structurizr.workspace.model.properties[
    ...
    function findOrCreateGroup(name, scope) {
        if (useNestedGroups()) {
            ...
            if (group === undefined) {
                if (name.indexOf(separator) > -1) {
                    var parentGroupName = name.substring(
                        0,
                        name.lastIndexOf(separator),
                    );
                    var groupName = name.substring(
                        name.lastIndexOf(separator) + separator.length,
                    );
    ...
    function addDoubleClickHandlerForElement(cellView, element) {
        ...
        if (
            element.type ===
            structurizr.constants.SOFTWARE_SYSTEM_INSTANCE_ELEMENT_TYPE
        ) {
            element = structurizr.workspace.findElementById(
        ...
        if (
            ...
            views = structurizr.workspace.findComponentViewsForContainer(
                element.id,
            );
        }

        views = views.concat(
            structurizr.workspace.findImageViewsForElement(element.id),
        );

        documentation =
            element.documentation &&
            element.documentation.sections &&
            element.documentation.sections.length > 0;
        decisions =
            element.documentation &&
            element.documentation.decisions &&
            element.documentation.decisions.length > 0;
        ...
        if (views.length > 0) {
            ...
            $("#" + cellView.id + " .structurizrZoom").attr(
                "transform",
        ...
        if (documentation) {
            ...
            $("#" + cellView.id + " .structurizrDocumentation").attr(
                "transform",
                "translate(" + translateX + " 0)",
            );
            translateX += translateXDelta;
        ...
        if (decisions) {
            ...
            $("#" + cellView.id + " .structurizrDecisions").attr(
                "transform",
                "translate(" + translateX + " 0)",
            );
            translateX += translateXDelta;
        ...
        if (url) {
            ...
            $("#" + cellView.id + " .structurizrLink").attr(
                "transform",
                "translate(" + translateX + " 0)",
            );
            translateX += translateXDelta;
    ...
    function addDoubleClickHandlerForRelationship(linkView, relationship) {
        ...
        if (url) {
            ...
            $("#" + linkView.id + " .structurizrNavigation")
                .parent()
                .css("color", linkView.model._computedStyle.color);
            $("#" + linkView.id + " .structurizrNavigation")
                .parent()
                .html(svg);
    ...
    function getPerspectiveForElement(element) {
        ...
        if (element.perspectives) {
            element.perspectives.forEach((perspective) => {
        ...
        if (p === undefined) {
            if (
                ...
                var softwareSystem = structurizr.workspace.findElementById(
                    element.softwareSystemId,
                );
                if (softwareSystem.perspectives) {
                    softwareSystem.perspectives.forEach((perspective) => {
                        if (perspective.name === currentPerspective) {
                            p = perspective;
                        }
                    });
                ...
                var container = structurizr.workspace.findElementById(
                    element.containerId,
                );
                if (container.perspectives) {
                    container.perspectives.forEach((perspective) => {
                        if (perspective.name === currentPerspective) {
                            p = perspective;
                        }
                    });
    ...
    function relationshipHasPerspective(relationship) {
        ...
        if (relationship.perspectives) {
            relationship.perspectives.forEach((perspective) => {
                if (perspective.name === currentPerspective) {
                    result = true;
                }
            });
        ...
        if (result === false) {
            if (relationship.linkedRelationshipId) {
                return relationshipHasPerspective(
                    structurizr.workspace.findRelationshipById(
    ...
    this.renderPerspectiveOrTagsFilter = () => {
        if (currentPerspective === undefined && currentTags.length === 0) {
            Object.keys(cellsByElementId).forEach((elementId) => {
                var cell = cellsByElementId[elementId];
                changeColourOfCell(
                    cell,
                    cell._computedStyle.background,
                    cell._computedStyle.color,
                    cell._computedStyle.stroke,
                );
        ...
        if (currentPerspective !== undefined) {
            Object.keys(cellsByElementId).forEach((elementId) => {
                ...
                const element = structurizr.workspace.findElementById(
                    cell.elementInView.id,
                );
                ...
                if (perspective === undefined) {
                    ...
                    const elementStyleTagForPerspective =
                        "Perspective:" + currentPerspective;
                    const elementStyles =
                        structurizr.workspace.views.configuration.styles
                            .elements;

                    // find a style for Perspective:Name
                    elementStyles.forEach((elementStyle) => {
                        if (
                            elementStyle.tag === elementStyleTagForPerspective
                        ) {
                    ...
                    elementStyles.forEach((elementStyle) => {
                        if (
                            elementStyle.tag.indexOf(
                                elementStyleTagForPerspective + "[value",
                            ) === 0
                        ) {
                            const expression = elementStyle.tag.substring(
                                elementStyle.tag.indexOf("[") + 1,
                                elementStyle.tag.length - 1,
                            );

                            if (expression.indexOf("value==") === 0) {
                                const testValue = expression.substring(
                                    "value==".length,
                                );
                    ...
                    if (elementStyleForPerspective !== undefined) {
                        ...
                        if (stroke === undefined) {
                            stroke = structurizr.util.shadeColor(
                                background,
                                darkenPercentage,
                                darkMode,
                            );
            ...
            lines.forEach((line) => {
                const relationship = structurizr.workspace.findRelationshipById(
                    line.relationshipInView.id,
                );
        ...
        if (currentTags.length > 0) {
            Object.keys(cellsByElementId).forEach((elementId) => {
                ...
                const tags =
                    structurizr.workspace.getAllTagsForElement(element);
            ...
            lines.forEach((line) => {
                var relationship = structurizr.workspace.findRelationshipById(
                    line.relationshipInView.id,
                );

                const tags =
                    structurizr.workspace.getAllTagsForRelationship(
                        relationship,
                    );
                if (hasTags(tags) === false) {
                    hideLine(relationship.id, "0.2");
    ...
    this.hasPerspective = () => currentPerspective !== undefined;
    ...
    this.hasTags = () => currentTags !== undefined;

    this.isEditable = () => editable;

    this.setEmbedded = (bool) => {
        embedded = bool;
    };

    this.isEmbedded = () => embedded;

    this.setNavigationEnabled = (bool) => {
        navigationEnabled = bool;
    };

    this.isNavigationEnabled = () => navigationEnabled;
    ...
    function setZoomAndMargins() {
        ...
        var maxMarginLeftRight = Math.max(
            0,
            (viewportWidth - minimumWidth) / 2,
        );
        canvas.css("margin-left", maxMarginLeftRight);
        canvas.css("margin-right", maxMarginLeftRight);

        var maxMarginTopBottom = Math.max(
            0,
            (viewportHeight - minimumHeight) / 2,
        );
    ...
    this.decreasePageSize = function (evt) {
        ...
        this.setPageSize(
            Math.max(diagramWidth - pageSizeDelta, diagramMetadataWidth),
            diagramHeight - pageSizeDelta,
        );
    ...
    this.getWidth = () => diagramWidth;

    this.getHeight = () => diagramHeight;

    function calculateElementPadding(configuration) {
        return configuration.width * 0.07;
    }

    function renderElementInternals(
        element,
        cell,
        configuration,
        width,
        ...
        const fill = structurizr.util.shadeColor(
            configuration.background,
            100 - configuration.opacity,
            darkMode,
        );
        const color = structurizr.util.shadeColor(
            configuration.color,
            100 - configuration.opacity,
            darkMode,
        );
        const stroke = structurizr.util.shadeColor(
        ...
        const heightOfNameLabel = calculateHeight(
            name,
            configuration.fontSize,
            nameFontSizeDifference,
            false,
        );
        const heightOfMetaDataLabel = calculateHeight(
            metadata,
            configuration.fontSize,
            metaDataFontSizeDifference,
            false,
        );
        const heightOfDescriptionLabel = calculateHeight(
            description,
            configuration.fontSize,
            0,
            false,
        ...
        if (totalY > height) {
            console.log(
                'The height of the element named "' +
                    element.name +
                    '" is too small to fit the content (' +
                    Math.ceil(totalY) +
                    "px)",
            );
        }
        ...
        if (configuration.icon) {
            ...
            cell.attributes.attrs[".structurizrIcon"]["xlink:href"] =
                configuration.icon;
            cell.attributes.attrs[".structurizrIcon"]["width"] = widthOfIcon;
            cell.attributes.attrs[".structurizrIcon"]["height"] =
                heightOfIcon - ICON_PADDING;
            cell.attributes.attrs[".structurizrIcon"]["ref-x"] = iconRefX;
            cell.attributes.attrs[".structurizrIcon"]["ref-y"] = iconRefY;
            cell.attributes.attrs[".structurizrIcon"]["opacity"] =
                configuration.opacity / 100;
        }
        ...
        cell.attributes.attrs[".structurizrName"]["font-size"] =
            configuration.fontSize + nameFontSizeDifference;
        ...
        cell.attributes.attrs[".structurizrMetaData"]["font-family"] =
            font.name;
        cell.attributes.attrs[".structurizrMetaData"]["fill"] = color;
        cell.attributes.attrs[".structurizrMetaData"]["font-size"] =
            configuration.fontSize + metaDataFontSizeDifference;
        cell.attributes.attrs[".structurizrMetaData"]["ref-y"] = metaDataRefY;
        cell.attributes.attrs[".structurizrMetaData"]["lineHeight"] =
            lineHeight;

        cell.attributes.attrs[".structurizrDescription"]["text"] = description;
        cell.attributes.attrs[".structurizrDescription"]["font-family"] =
            font.name;
        cell.attributes.attrs[".structurizrDescription"]["fill"] = color;
        cell.attributes.attrs[".structurizrDescription"]["font-size"] =
            configuration.fontSize;
        cell.attributes.attrs[".structurizrDescription"]["ref-y"] =
            descriptionRefY;
        cell.attributes.attrs[".structurizrDescription"]["lineHeight"] =
            lineHeight;

        cell.attributes.attrs[".structurizrNavigation"]["color"] =
            navigationColor;
        cell.attributes.attrs[".structurizrNavigation"]["ref-y"] =
            navigationRefY;
    ...
    function createBox(view, element, configuration, x, y, cornerRadius) {
        ...
        var fill = structurizr.util.shadeColor(
            configuration.background,
            100 - configuration.opacity,
            darkMode,
        );
        var stroke = structurizr.util.shadeColor(
            configuration.stroke,
            100 - configuration.opacity,
            darkMode,
        );
        ...
        if (configuration.border !== "Solid") {
            cell.attributes.attrs[".structurizrBox"]["stroke-dasharray"] =
                borderStyles[configuration.border];
        }
    ...
    function createEllipse(view, element, configuration, x, y, circle) {
        ...
        if (configuration.border !== "Solid") {
            cell.attributes.attrs[".structurizrEllipse"]["stroke-dasharray"] =
                borderStyles[configuration.border];
        }
    ...
    function createHexagon(view, element, configuration, x, y) {
        ...
        if (configuration.border !== "Solid") {
            cell.attributes.attrs[".structurizrHexagon"]["stroke-dasharray"] =
                borderStyles[configuration.border];
        }
    ...
    function createDiamond(view, element, configuration, x, y) {
        ...
        if (configuration.border !== "Solid") {
            cell.attributes.attrs[".structurizrDiamond"]["stroke-dasharray"] =
                borderStyles[configuration.border];
        }
    ...
    function createPerson(view, element, configuration, x, y) {
        ...
        if (configuration.border !== "Solid") {
            cell.attributes.attrs[".structurizrPersonHead"][
                "stroke-dasharray"
            ] = borderStyles[configuration.border];
            cell.attributes.attrs[".structurizrPersonBody"][
                "stroke-dasharray"
            ] = borderStyles[configuration.border];
            cell.attributes.attrs[".structurizrPersonRightArm"][
                "stroke-dasharray"
            ] = borderStyles[configuration.border];
            cell.attributes.attrs[".structurizrPersonLeftArm"][
                "stroke-dasharray"
            ] = borderStyles[configuration.border];
        }
    ...
    function createRobot(view, element, configuration, x, y) {
        ...
        if (configuration.border !== "Solid") {
            cell.attributes.attrs[".structurizrRobotHead"]["stroke-dasharray"] =
                borderStyles[configuration.border];
            cell.attributes.attrs[".structurizrRobotBody"]["stroke-dasharray"] =
                borderStyles[configuration.border];
            cell.attributes.attrs[".structurizrRobotRightArm"][
                "stroke-dasharray"
            ] = borderStyles[configuration.border];
            cell.attributes.attrs[".structurizrRobotLeftArm"][
                "stroke-dasharray"
            ] = borderStyles[configuration.border];
        }
    ...
    function createCylinder(view, element, configuration, x, y) {
        ...
        if (configuration.border !== "Solid") {
            cell.attributes.attrs[".structurizrCylinderPath"][
                "stroke-dasharray"
            ] = borderStyles[configuration.border];
        }
    ...
    function createPipe(view, element, configuration, x, y) {
        ...
        if (configuration.border !== "Solid") {
            cell.attributes.attrs[".structurizrPipePath"]["stroke-dasharray"] =
                borderStyles[configuration.border];
        }
    ...
    function createFolder(view, element, configuration, x, y) {
        ...
        if (configuration.border !== "Solid") {
            cell.attributes.attrs[".structurizrFolderTab"]["stroke-dasharray"] =
                borderStyles[configuration.border];
            cell.attributes.attrs[".structurizrFolder"]["stroke-dasharray"] =
                borderStyles[configuration.border];
        }
    ...
    function createComponent(view, element, configuration, x, y) {
        ...
        if (configuration.border !== "Solid") {
            cell.attributes.attrs[".structurizrComponent"]["stroke-dasharray"] =
                borderStyles[configuration.border];
            cell.attributes.attrs[".structurizrComponentBlockTop"][
                "stroke-dasharray"
            ] = borderStyles[configuration.border];
            cell.attributes.attrs[".structurizrComponentBlockBottom"][
                "stroke-dasharray"
            ] = borderStyles[configuration.border];
        }
    ...
    function createWebBrowser(view, element, configuration, x, y) {
        ...
        if (configuration.border !== "Solid") {
            cell.attributes.attrs[".structurizrWebBrowser"][
                "stroke-dasharray"
            ] = borderStyles[configuration.border];
        }
        ...
        renderElementInternals(
            ...
            cell,
            configuration,
            webBrowserPanelWidth,
            webBrowserPanelWidth * 0.1,
            webBrowserPanelHeight,
            0,
        );
    ...
    function createWindow(view, element, configuration, x, y) {
        ...
        if (configuration.border !== "Solid") {
            cell.attributes.attrs[".structurizrWindow"]["stroke-dasharray"] =
                borderStyles[configuration.border];
        }
    ...
    function createMobileDevicePortrait(view, element, configuration, x, y) {
        ...
        var cell = new structurizr.shapes.MobileDevice({
            ...
            attrs: {
                ...
                ".structurizrMobileDeviceSpeaker": {
                    ...
                    x2:
                        configuration.width -
                        (configuration.width - speakerLength) / 2,
                    y2: 20,
                },
            },
            element: element,
        ...
        if (configuration.border !== "Solid") {
            cell.attributes.attrs[".structurizrMobileDevice"][
                "stroke-dasharray"
            ] = borderStyles[configuration.border];
        }
            ...
            attrs: {
                ...
                ".structurizrMobileDeviceSpeaker": {
                    ...
                    y2:
                        ...
                        (configuration.height - speakerLength) / 2,
                },
            },
            element: element,
        });
    ...
    function createDiagramMetadata() {
        const viewOrFilter =
            currentFilter !== undefined ? currentFilter : currentView;
        ...
        diagramTitle.toBack();
        diagramMetadataWidth = Math.max(
            diagramMetadataWidth,
            calculateWidth(title, elementStyleForDiagramTitle.fontSize),
        );
            ...
            diagramDescription = new structurizr.shapes.DiagramDescription({
                attrs: {
                    ".structurizrDiagramDescription": {
                        text: description,
                        "font-size":
                            elementStyleForDiagramDescription.fontSize + "px",
                        "font-family": font.name,
        ...
        const timezone =
            structurizr.workspace.views.configuration.properties[
                "structurizr.timezone"
            ];
        const locale =
            structurizr.workspace.views.configuration.properties[
                "structurizr.locale"
        ...
        if (structurizr.workspace.id === 0) {
            ...
            if (lastModified) {
                metadata = new Date(lastModified).toLocaleString(
                    locale,
                    options,
                );
            }
        ...
        diagramMetadataWidth = Math.max(
            diagramMetadataWidth,
            calculateWidth(metadata, elementStyleForDiagramMetadata.fontSize),
        );
        ...
        const showDescription =
            getViewOrViewSetProperty(
                viewOrFilter,
                "structurizr.description",
                "true",
            ) === "true";
        const showMetadata =
            getViewOrViewSetProperty(
                viewOrFilter,
                "structurizr.metadata",
                "true",
            ) === "true";

        const titleHeight = calculateHeight(
            "Title",
            elementStyleForDiagramTitle.fontSize,
            0,
            false,
        );
        const descriptionHeight = calculateHeight(
            "Description",
            elementStyleForDiagramDescription.fontSize,
            0,
            false,
        );
        const metadataHeight = calculateHeight(
            "Metadata",
            elementStyleForDiagramMetadata.fontSize,
            0,
            false,
            ...
            brandingLogo = new structurizr.shapes.BrandingImage({
                ...
                attrs: {
                    image: {
                        ...
                        width:
                            getImageRatio(branding.logo) *
                            diagramMetadataHeight,
                        height: diagramMetadataHeight,
                    },
                },
            });
    ...
    function getViewOrViewSetProperty(view, name, defaultValue) {
        ...
        if (
            structurizr.workspace.views.configuration.properties &&
            structurizr.workspace.views.configuration.properties[name]
        ) {
    ...
    function calculateHeight(text, fontSize, fontSizeDelta, addPadding) {
        ...
        if (text) {
            ...
            if (text.length === 0) {
                ...
                return (
                    numberOfLines * ((fontSize + fontSizeDelta) * lineSpacing)
                );
            }
        } else {
            return 0;
    ...
    function formatName(element, configuration, horizontalPadding) {
        return breakText(
            ...
            Math.max(0, configuration.width - horizontalPadding),
            font.name,
            configuration.fontSize + nameFontSizeDifference,
        );
    }
    ...
    function formatMetaData(element, configuration, horizontalPadding) {
        ...
        if (
            element.type === "Custom" &&
            (element.metadata === undefined || element.metadata === "")
        ) {
            return "";
        }
        ...
        if (
            ...
            configuration.metadata === false
        ) {
            return "";
        } else {
            var metadata = breakText(
                structurizr.ui.getMetadataForElement(element, true),
                configuration.width - horizontalPadding,
                font.name,
                configuration.fontSize + metaDataFontSizeDifference,
            );
            ...
            if (currentView.type === "Deployment") {
                if (element.type === "ContainerInstance") {
                    ...
                    if (container) {
                        if (
                            currentView.softwareSystemId === undefined ||
                            container.parentId !== currentView.softwareSystemId
                        ) {
    ...
    function removeIllegalElements() {
        try {
            if (currentView.type === "Container") {
                ...
                currentView.elements.forEach((elementInView) => {
                    var element = structurizr.workspace.findElementById(
                        elementInView.id,
                    );
                    if (
                        element.type === "Container" &&
                        element.parentId !== currentView.softwareSystemId
                    ) {
                        if (
                            softwareSystemIdsToRemove.indexOf(
                                element.parentId,
                            ) === -1
                        ) {
                ...
                for (var i = 0; i < softwareSystemIdsToRemove.length; i++) {
                    for (var j = 0; j < currentView.elements.length; j++) {
                        if (
                            currentView.elements[j].id ===
                            softwareSystemIdsToRemove[i]
                        ) {
                            console.log(
                                "Removing software system with ID " +
                                    softwareSystemIdsToRemove[i] +
                                    " because one of it's child containers is included on the diagram",
                            );
            ...
            if (currentView.type === "Component") {
                ...
                currentView.elements.forEach((elementInView) => {
                    ...
                    if (
                        ...
                        element.parentId !== currentView.containerId
                    ) {
                        if (
                            containerIdsToRemove.indexOf(element.parentId) ===
                            -1
                        ) {
                            containerIdsToRemove.push(element.parentId);
                        }

                        var foreignContainer =
                            structurizr.workspace.findElementById(
                                element.parentId,
                            );
                        if (
                            foreignContainer &&
                            softwareSystemIdsToRemove.indexOf(
                                foreignContainer.parentId,
                            ) === -1
                        ) {
                            softwareSystemIdsToRemove.push(
                                foreignContainer.parentId,
                            );
                        }
                    } else if (
                        element.type === "Container" &&
                        element.id !== currentView.containerId
                    ) {
                        if (
                            softwareSystemIdsToRemove.indexOf(
                                element.parentId,
                ...
                for (var i = 0; i < containerIdsToRemove.length; i++) {
                    for (var j = 0; j < currentView.elements.length; j++) {
                        if (
                            currentView.elements[j].id ===
                            containerIdsToRemove[i]
                        ) {
                            console.log(
                                "Removing container with ID " +
                                    containerIdsToRemove[i] +
                                    " because one of it's child components is included on the diagram",
                            );
                            currentView.elements.splice(j, 1);
                        }
                    }
    ...
    function includeRelationshipOnDiagram(relationship) {
        return includeOnDiagram(
            structurizr.workspace.getAllTagsForRelationship(relationship),
        );
    }
    ...
    function createArrow(relationshipInView) {
        ...
        if (
            mapOfIdToBox[relationship.sourceId] &&
            mapOfIdToBox[relationship.destinationId]
        ) {
            var configuration;

            if (
                currentView.type === "Dynamic" &&
                relationshipInView.response !== undefined &&
                relationshipInView.response === true
            ) {
                var originalTags = relationship.tags;
                relationship.tags =
                    relationship.tags + ",Relationship/Response";
                configuration = structurizr.ui.findRelationshipStyle(
                    relationship,
                    darkMode,
                );
            ...
            if (configuration.style === "Dashed") {
                ...
                strokeDashArray = calculateStrokeDottedArray(
                    configuration.thickness,
                );
            }
            ...
            var heightOfDescription = calculateHeight(
                description,
                configuration.fontSize,
                0,
                false,
            ...
            if (technology && technology.trim().length > 0) {
                ...
                var heightOfTechnology = calculateHeight(
                    technology,
                    configuration.fontSize,
                    metaDataFontSizeDifference,
                    false,
                ...
                verticalOffset +=
                    heightOfTechnology - configuration.fontSize / 2;
            }
            ...
            if (
                configuration.style === undefined ||
                configuration.style === "Dashed" ||
                configuration.style === "Dotted"
            ) {
                link.attr({
                    ".connection": {
                        stroke: fill,
            ...
            if (relationshipInView.order) {
                mapOfIdToLine[
                    relationshipInView.id + "/" + relationshipInView.order
                ] = link;
            } else {
            ...
            if (navigationEnabled) {
                addDoubleClickHandlerForRelationship(
                    paper.findViewByModel(link),
                    relationship,
                );
            }
            ...
            console.log(
                "Not rendering relationship " +
                    relationship.id +
                    " (" +
                    structurizr.workspace.findElementById(relationship.sourceId)
                        .name +
                    " -> " +
                    structurizr.workspace.findElementById(
                        relationship.destinationId,
                    ).name +
                    ") because the source and destination elements are not on the diagram.",
            );
        }
    }
        ...
        switch (siblings.length) {
            ...
            default:
                ...
                siblings.forEach((sibling) => {
                    if (
                        sibling.relationshipInView.vertices === undefined ||
                        sibling.relationshipInView.vertices.length === 0
                    ) {
                ...
                if (numberOfSiblingsWithoutVertices >= 2) {
                    ...
                    var midPoint = joint.g
                        .line(srcCenter, trgCenter)
                        .midpoint();
                    ...
                    _.each(siblings, (sibling, index) => {
                        ...
                        // We found the vertex.
                        var vertex = joint.g.point.fromPolar(
                            offset,
                            angle,
                            midPoint,
                        );
    ...
    function createBoundary(name, metadata, type, element) {
        ...
        if (type === "Group") {
            ...
            // and apply opacity
            textColor = structurizr.util.shadeColor(
                textColor,
                100 - elementStyle.opacity,
                darkMode,
            );
            stroke = structurizr.util.shadeColor(
                stroke,
            ...
            const elementStyleForBoundaryElement =
                structurizr.ui.findElementStyle(element);
            strokeWidth = elementStyle.strokeWidth;
        ...
        if (metadata !== undefined) {
            ...
            if (
                ...
                elementStyle.metadata !== undefined &&
                elementStyle.metadata === false
            ) {
                metadata = "";
            }
        } else {
        ...
        if (icon) {
            ...
            boundary.attributes.attrs[".structurizrIcon"]["width"] =
                widthOfIcon;
            boundary.attributes.attrs[".structurizrIcon"]["height"] =
                heightOfIcon;
            boundary.attributes.attrs[".structurizrIcon"]["opacity"] =
    ...
    function createDeploymentNode(element) {
        ...
        var textColor = structurizr.util.shadeColor(
            configuration.color,
            100 - configuration.opacity,
            darkMode,
        );
        ...
        if (configuration.border !== "Solid") {
            cell.attributes.attrs[".structurizrDeploymentNode"][
                "stroke-dasharray"
            ] = borderStyles[configuration.border];
        }
    ...
    function moveLinksBetweenElementsContainedWithin(cell, newPosition, opt) {
        if (opt.translateBy === undefined) {
            ...
            if (cell === translatedByCell) {
                ...
                var embeddedCells = cell
                    .getEmbeddedCells({ deep: true })
                    .map((cell) => paper.findViewByModel(cell));
    ...
    this.setPaperSize = function (view) {
        if (view.dimensions !== undefined) {
            if (view.paperSize !== undefined) {
                var dimensions = new structurizr.ui.PaperSizes().getDimensions(
                    view.paperSize,
                );
                if (
                    dimensions.width === view.dimensions.width &&
                    dimensions.height === view.dimensions.height
                ) {
                ...
                view.dimensions.width = Math.max(
    ...
    function reposition(parentCell) {
        ...
        if (parentCell && parentCell.getEmbeddedCells().length > 0) {
            ...
            if (
                parentCell.elementInView &&
                parentCell.positionCalculated === false
            ) {
                // this is an element from the model
                var element = structurizr.workspace.findElementById(
                    parentCell.elementInView.id,
                );
        ...
        if (links) {
            links.forEach((link) => {
                ...
                if (oldVertices) {
                    ...
                    if (moved) {
                        previousPositions.push(
                            getCurrentLinkPositions([link])[0],
                        );
    ...
    this.zoomFitContent = function () {
        ...
        var centreX =
            contentArea.minX + (contentArea.maxX - contentArea.minX) / 2;
        var centreY =
            contentArea.minY + (contentArea.maxY - contentArea.minY) / 2;
    ...
    function zoomToAndScroll(zoomScale, evt) {
        ...
        var localPointBeforeZooming = paper.clientToLocalPoint(
            clientTarget.x,
            clientTarget.y,
        );
        ...
        self.scrollToPoint(
            localPointBeforeZooming.x,
            localPointBeforeZooming.y,
            clientTarget.x,
            clientTarget.y,
        );
    }
    ...
    this.scrollToPoint = (paperX, paperY, clientX, clientY) => {
    ...
    this.getPossibleViewportWidth = () => parentElement.innerWidth();

    this.getPossibleViewportHeight = () => parentElement.innerHeight();
    ...
    this.onElementsSelected = (callback) => {
        elementsSelectedEventHandler = callback;
    };

    this.onElementDoubleClicked = (callback) => {
        elementDoubleClickedHandler = callback;
    };

    this.onRelationshipDoubleClicked = (callback) => {
        relationshipDoubleClickedHandler = callback;
    };

    function fireElementsSelectedEvent() {
        if (elementsSelectedEventHandler) {
            elementsSelectedEventHandler(
                selectedElements.map((cell) => cell.model.elementInView.id),
            );
        }
    }
    ...
    function selectElement(cellView) {
        ...
        var structurizrBox = $(
            "#" + cellView.el.id + " .structurizrHighlightableElement",
        );
    ...
    this.selectElementsWithName = (regex) => {
        var filter = new RegExp(regex);

        graph.getElements().forEach((element) => {
            if (element.elementInView && element.positionCalculated === false) {
                var elementInModel = structurizr.workspace.findElementById(
                    element.elementInView.id,
                );
    ...
    function deselectElement(cellView) {
        ...
        if (highlightedElement > -1) {
            structurizrBox.attr(
                "class",
                classes.substring(0, highlightedElement),
            );
        }
    ...
    function getImageMetadata(image) {
        return imageMetadata.filter((im) => im.src === image)[0];
    }
    ...
    function createDiagramKey() {
        ...
        var elementsInView = currentView.elements.map((element) => structurizr.workspace.findElementById(element.id));
        for (var i = 0; i < elementsInView.length; i++) {
            var elementInView = elementsInView[i];
            if (elementInView.type === "DeploymentNode") {
                var elementStyle = structurizr.ui.findElementStyle(
                    elementInView,
                    darkMode,
                );
        ...
        elementStylesInUse.sort((a, b) => a.localeCompare(b));

        var relationshipStylesInUse = [];
        var relationshipStylesInUseMap = {};
        var relationshipsInView = currentView.relationships.map(
            (relationship) => structurizr.workspace.findRelationshipById(
                    relationship.id,
                ),
        );
        for (var i = 0; i < relationshipsInView.length; i++) {
            var relationshipInView = relationshipsInView[i];
            var relationshipStyle = structurizr.ui.findRelationshipStyle(
                relationshipInView,
                darkMode,
            );
            var relationshipStyleIdentifier = createTagsList(
                relationshipStyle,
                "Relationship",
            );
            if (
                relationshipStylesInUse.indexOf(relationshipStyleIdentifier) ===
                -1
            ) {
                relationshipStylesInUse.push(relationshipStyleIdentifier);
                relationshipStylesInUseMap[relationshipStyleIdentifier] =
                    relationshipStyle;
            }
        }

        relationshipStylesInUse.sort((a, b) => a.localeCompare(b));

        var numberOfItemsInKey =
            elementStylesInUse.length + relationshipStylesInUse.length;
        if (currentView.type === "Deployment") {
            numberOfItemsInKey++;
        }
        var columns = 5;
            ...
            if (elementStyle.shape === "RoundedBox") {
                ...
                svg +=
                    ...
                    (elementStyle.border !== "Solid"
                        ? ' stroke-dasharray="' +
                          borderStylesForKey[elementStyle.border] +
                          '"'
                        : "") +
                    "/>";
                svg += createTextForKey(
                    width,
                    height,
                    0,
                    0,
                    createTagsList(elementStyle, "Element"),
                    undefined,
                    textColor,
                    elementStyle.icon,
                ...
                var lidRadius = 45;
                var uniqueKey =
                    "key" + elementStyle.tag.replace(/ /g, "") + "Cylinder";
                var width = keyElementWidth;
                var height =
                    keyElementWidth *
                    (elementStyle.height / elementStyle.width);
                ...
                var lidRadius = 45;
                var uniqueKey =
                    "key" + elementStyle.tag.replace(/ /g, "") + "Pipe";
                var width = keyElementWidth;
                var height = keyElementHeight;
                ...
                svg +=
                    ...
                    '" height="' +
                    height +
                    '" rx="10" ry="10" x="0" y="0" fill="' +
                    stroke +
                    '" stroke-width="5" stroke="' +
                    stroke +
                    '"' +
                    (elementStyle.border !== "Solid"
                        ? ' stroke-dasharray="' +
                ...
                svg +=
                    ...
                    '" height="' +
                    (height - 50) +
                    '" rx="10" ry="10" x="10" y="40" fill="' +
                    fill +
                    '" stroke-width="0"/>';
                svg +=
                    '<rect width="' +
                    (width - 110) +
                    '" height="20" rx="10" ry="10" x="100" y="10" fill="' +
                    fill +
                    '" stroke-width="0"/>';
                svg +=
                    '<ellipse cx="20" cy="20" rx="10" ry="10" fill="' +
                    fill +
                    '" stroke-width="0"/>';
                svg +=
                    '<ellipse cx="50" cy="20" rx="10" ry="10" fill="' +
                    fill +
                    '" stroke-width="0"/>';
                svg +=
                    '<ellipse cx="80" cy="20" rx="10" ry="10" fill="' +
                    fill +
                    '" stroke-width="0"/>';
                svg += createTextForKey(
                    width,
        ...
        for (var i = 0; i < relationshipStylesInUse.length; i++) {
            relationshipStyle =
                relationshipStylesInUseMap[relationshipStylesInUse[i]];
            var strokeDashArray;
            ...
            svg +=
                ...
                '" stroke-dasharray="" />';
            if (
                relationshipStyle.style === "Dashed" ||
                relationshipStyle.style === "Dotted"
            ) {
                svg +=
                    '<path d="M0,30 L' +
                    (width - 60) +
            ...
            svg += createTextForKey(
                ...
                0,
                60,
                createTagsList(relationshipStyle, "Relationship"),
                undefined,
                fill,
            );
            svg += "</g>";
    ...
    function createSvgGroup(
        ...
        var offsetX =
            (column - 1) * columnWidth + (columnWidth - contentWidth) / 2;
        var offsetY = (row - 1) * rowHeight + (rowHeight - contentHeight) / 2;

        return '<g transform="translate(' + offsetX + "," + offsetY + ')">';
    }

    function createTextForKey(
        width,
        height,
        offsetX,
        offsetY,
        ...
        var text = breakText(
            structurizr.util.escapeHtml(tag),
            width * 0.8,
            font.name,
            fontSize,
        );
    ...
    function findContentArea(crop, margin) {
        if (
            currentView &&
            (currentView.elements === undefined ||
        ...
        for (var i = 0; i < graph.getElements().length; i++) {
            ...
            if (
                cell.elementInView !== undefined ||
                cell.attributes.type === "structurizr.boundary"
            ) {
    ...
    function convertSvgToPng(includeDiagramMetadata, crop, callback) {
        var svgMarkup = self.exportCurrentDiagramToSVG(
            includeDiagramMetadata,
            false,
        );
        ...
        svgMarkup = svgMarkup.replace(
            /class="marker-vertices"/g,
            'class="marker-vertices" display="none"',
        );
    ...
    this.exportCurrentDiagramToSVG = function (
        includeDiagramMetadata,
        includeFont,
    ) {
    ...
    this.exportCurrentDiagramKeyToSVG = (includeFont) => {
    ...
    this.exportCurrentDiagramToPNG = function (
        ...
        if (callback) {
            convertSvgToPng(includeDiagramMetadata, crop, (png) => {
                $(".structurizrNavigation").attr("display", "block");
    ...
    this.exportCurrentDiagramKeyToPNG = (callback) => {
    ...
    this.exportCurrentThumbnailToPNG = (callback) => {
        try {
            convertSvgToPng(true, false, (exportedImage) => {
                $(".structurizrNavigation").attr("display", "block");
                resizeImage(
                    exportedImage,
                    thumbnailWidth,
                    Math.floor(diagramHeight / (diagramWidth / thumbnailWidth)),
                    callback,
                );
            });
        } catch (err) {
    ...
    this.currentViewIsDynamic = () => currentView.type === "Dynamic";

    this.currentViewHasAnimation = () => (
            currentView.animations !== undefined &&
            currentView.animations.length > 1
        );

    this.animationStarted = () => animationStarted;

    this.onAnimationStarted = (f) => {
        animationStartedEventHandler = f;
    };

    this.onAnimationStopped = (f) => {
    ...
    this.startAnimation = function (autoPlay) {
        ...
        if (this.currentViewIsDynamic()) {
            ...
            linesToAnimate.sort((a, b) => a.relationshipInView.order - b.relationshipInView.order);
            animationIndex = 0;
            ...
            animationSteps.sort((a, b) => a.order - b.order);
            animationIndex = 0;
        ...
        if (this.currentViewIsDynamic()) {
            if (this.animationStarted()) {
                if (animationIndex > 0) {
                    ...
                    var order =
                        linesToAnimate[animationIndex].relationshipInView.order;
                    while (
                        animationIndex >= 0 &&
                        linesToAnimate[animationIndex].relationshipInView
                            .order === order
                    ) {
                        animationIndex--;
                    }
                ...
                if (animationIndex === 1) {
                    ...
                    if (animationStep) {
                        if (animationStep.elements) {
                            animationStep.elements.forEach(
                                (elementId) => {
                        ...
                        if (animationStep.relationships) {
                            animationStep.relationships.forEach(
                                (relationshipId) => {
    ...
    this.continueAnimation = function (autoPlay) {
        if (this.currentViewIsDynamic()) {
            ...
            if (animationStep) {
                if (animationStep.elements) {
                    animationStep.elements.forEach((elementId) => {
                        showElement(elementId);
                    });
                }
                if (animationStep.relationships) {
    ...
    function fadeAllElements() {
        ...
        cells.forEach((cell) => {
            var background = structurizr.util.shadeColor(
                cell._computedStyle.background,
                shadePercentage,
                darkMode,
            );
            var color = structurizr.util.shadeColor(
                cell._computedStyle.color,
                shadePercentage,
                darkMode,
            );
    ...
    function showElement(elementId) {
        ...
        if (element) {
            ...
            $("#" + elementView.el.id + " .structurizrElement").css(
                "opacity",
                "1.0",
            );
        }
    ...
    function unfadeAllElements() {
        Object.keys(mapOfIdToBox).forEach((elementId) => {
    ...
    this.changeColorOfElement = (elementId, background) => {
        var cell = mapOfIdToBox[elementId];
        if (cell) {
            var foreground = cell._computedStyle.color;
            var stroke = structurizr.util.shadeColor(
    ...
    function highlightLinesWithOrder(order) {
        var line = linesToAnimate[animationIndex];
        while (
            animationIndex < linesToAnimate.length &&
            line.relationshipInView.order === order
        ) {
    ...
    this.hasElementsSelected = () => selectedElements.length > 0;

    this.hasElementHighlighted = () => highlightedElement !== undefined;

    this.getHighlightedElementId = () => highlightedElement.model.elementInView.id;

    this.hasLinkHighlighted = () => highlightedLink !== undefined;

    this.getHighlightedRelationshipId = () => highlightedLink.model.relationshipInView.id;

    this.moveLabelOfHighlightedLink = (delta) => {
        ...
        if (labels) {
            if (labels[0]) {
                ...
                var newDistance = Math.min(
                    1,
                    Math.max(0, currentDistance + delta),
                );
                ...
                highlightedLink.model.relationshipInView.position = Math.round(
                    newDistance * 100,
                );
                fireWorkspaceChangedEvent();
            }
    ...
    this.toggleRoutingOfHighlightedLink = () => {
        if (highlightedLink.model.relationshipInView.routing === undefined) {
            ...
            var relationship = structurizr.workspace.findRelationshipById(
                highlightedLink.model.relationshipInView.id,
            );
            if (relationship) {
                var configuration = structurizr.ui.findRelationshipStyle(
                    relationship,
                    darkMode,
                );
                highlightedLink.model.relationshipInView.routing = undefined;
                setRoutingAndConnector(
                    highlightedLink.model,
    ...
    this.moveSelectedElementsLeft = () => {
        addToUndoBuffer(
            getCurrentElementPositions(selectedElements).concat(
                getCurrentLinkPositions(getLinksBetweenSelectedElements()),
            ),
        );
        selectedElements.forEach((cellView) => {
    ...
    this.moveSelectedElementsRight = () => {
        addToUndoBuffer(
            getCurrentElementPositions(selectedElements).concat(
                getCurrentLinkPositions(getLinksBetweenSelectedElements()),
            ),
    ...
    this.alignSelectedElementsRight = function () {
        if (this.hasElementsSelected()) {
            ...
            var firstSelectedElement = selectedElements[0].model;
            var right =
                firstSelectedElement.get("position").x +
                firstSelectedElement.get("size").width;
    ...
    this.alignSelectedElementsVerticalCentre = function () {
        if (this.hasElementsSelected()) {
            ...
            var centre =
                firstSelectedElement.get("position").x +
                firstSelectedElement.get("size").width / 2;
    ...
    this.alignSelectedElementsBottom = function () {
        if (this.hasElementsSelected()) {
            ...
            var firstSelectedElement = selectedElements[0].model;
            var bottom =
                firstSelectedElement.get("position").y +
                firstSelectedElement.get("size").height;
    ...
    this.alignSelectedElementsHorizontalCentre = function () {
        if (this.hasElementsSelected()) {
            ...
            var centre =
                firstSelectedElement.get("position").y +
                firstSelectedElement.get("size").height / 2;
    ...
    this.distributeSelectedElementsHorizontally = function () {
        if (this.hasElementsSelected()) {
            ...
            var totalWidth =
                lastSelectedElement.model.get("position").x +
                lastSelectedElement.model.get("size").width -
                firstSelectedElement.model.get("position").x;
            var totalWidthOfElements = 0;
            selectedElements.forEach((cellView) => {
                totalWidthOfElements += cellView.model.get("size").width;
            });
            var spacingBetweenElements = Math.floor(
                (totalWidth - totalWidthOfElements) /
                    (selectedElements.length - 1),
            );
            ...
            for (var i = 1; i < selectedElements.length - 1; i++) {
                ...
                x +=
                    previousCellView.model.get("size").width +
    ...
    this.distributeSelectedElementsVertically = function () {
        if (this.hasElementsSelected()) {
            ...
            var totalHeight =
                lastSelectedElement.model.get("position").y +
                lastSelectedElement.model.get("size").height -
                firstSelectedElement.model.get("position").y;
            var totalHeightOfElements = 0;
            selectedElements.forEach((cellView) => {
                totalHeightOfElements += cellView.model.get("size").height;
            });
            var spacingBetweenElements = Math.floor(
                (totalHeight - totalHeightOfElements) /
                    (selectedElements.length - 1),
            );
            ...
            for (var i = 1; i < selectedElements.length - 1; i++) {
                ...
                y +=
                    previousCellView.model.get("size").height +
                    spacingBetweenElements;
    ...
    this.centreElements = (elements) => {
        var minX = diagramWidth,
            minY = diagramHeight;
        var maxX = 0,
            maxY = 0;
        elements.forEach((cellView) => {
            var x = cellView.model.get("position").x;
            var y = cellView.model.get("position").y;
            var width = cellView.model.get("size").width;
            var height = cellView.model.get("size").height;
        ...
        graph.getElements().forEach((cell) => {
            if (cell.elementInView && cell.positionCalculated === false) {
                ...
                elementBoundingBox.right =
                    elementBoundingBox.left + elementBoundingBox.width;
                elementBoundingBox.bottom =
                    elementBoundingBox.top + elementBoundingBox.height;
    ...
    function moveLinksBetweenElements(elements, dx, dy) {
        ...
        linksBetweenElements.forEach((link) => {
            var oldVertices = link.get("vertices");
            if (oldVertices) {
                var newVertices = [];
    ...
    function getLinksBetweenElements(elements) {
        ...
        if (links) {
            links.forEach((link) => {
                var sourceCellView = paper.findViewByModel(
                    graph.getCell(link.get("source")),
                );
                var targetCellView = paper.findViewByModel(
                    graph.getCell(link.get("target")),
                );

                if (
                    elements.indexOf(sourceCellView) > -1 &&
                    elements.indexOf(targetCellView) > -1
                ) {
    ...
    function addPaperEventHandlers() {
        paper.on("cell:mouseover", (cell, evt) => {
            ...
            if (cell.model.relationshipInView) {
                ...
                var point = V(paper.viewport).toLocalPoint(
                    evt.clientX,
                    evt.clientY,
                );
                currentX = point.x;
                ...
                if (tooltip && tooltip.isEnabled()) {
                    if (cell.model.elementInView) {
                        showTooltipForElement(
                            structurizr.workspace.findElementById(
                                cell.model.elementInView.id,
                            ),
                            cell.model._computedStyle,
                            x,
                            y,
                        );
                    } else if (cell.model.relationshipInView) {
                        showTooltipForRelationship(
                            structurizr.workspace.findRelationshipById(
                                cell.model.relationshipInView.id,
                            ),
                            cell.model.relationshipInView,
                            cell.model._computedStyle,
                            x,
                            y,
                        );
        ...
        paper.on("cell:mouseout", (cell, evt) => {
        ...
        paper.on("cell:pointerdown", (cell, evt, x, y) => {
            if (!cell.getConnectionLength) {
                ...
                if (self.hasElementsSelected()) {
                    if (selectedElements.indexOf(cell) === -1) {
                        ...
                        previousPositions = getCurrentElementPositions(
                            selectedElements,
                        ).concat(
                            getCurrentLinkPositions(
                                getLinksBetweenSelectedElements(),
                            ),
                        );
                    }
                } else {
        ...
        paper.on("cell:pointerup", (cell, evt, x, y) => {
                ...
                if (!cell.getConnectionLength) {
                    previousPositions.forEach((previousPosition) => {
                        if (previousPosition.element === cell.model) {
                            if (
                                previousPosition.x !==
                                    cell.model.position().x ||
                                previousPosition.y !== cell.model.position().y
                            ) {
                                addToUndoBuffer(previousPositions);
                            }
                        }
        ...
        paper.on("cell:pointerclick", (cellView, evt, x, y) => {
            if (self.isEditable()) {
                if (
                    cellView.model.elementInView &&
                    cellView.model.positionCalculated === false
                ) {
    ...
    this.setKeyboardShortcutsEnabled = (bool) => {
        keyboardShortcutsEnabled = bool;
    };

    this.areKeyboardShortcutsEnabled = () => keyboardShortcutsEnabled;

    function getCurrentElementPositions(elementViews) {
        var previousPositions = [];
        elementViews.forEach((elementView) => {
            var allElements = elementView.model.getEmbeddedCells({
                deep: true,
            });
    ...
    this.runDagre = function (
        ...
        try {
            ...
            addToUndoBuffer(
                getCurrentElementPositions(cellViews).concat(
                    getCurrentLinkPositions(graph.getLinks()),
                ),
            );
            ...
            console.error(
                "There was an error applying the automatic layout: " + err,
            );
        }
    };
    ...
    function centreDiagram() {
        ...
        var dx = Math.floor(
            (diagramWidth - contentArea.maxX - contentArea.minX) / 2,
        );
        var dy = Math.floor(
            (diagramHeight - contentArea.maxY - contentArea.minY) / 2,
        );
    ...
    this.undoStackIsEmpty = () => undoStack.isEmpty();

    function showTooltipForElement(element, style, x, y) {
        if (
            currentPerspective !== undefined &&
            elementHasPerspective(element) === false
        ) {
            return;
        }
        ...
        if (currentPerspective !== undefined) {
            ...
            if (perspectiveDetails === undefined) {
                if (element.type === "SoftwareSystemInstance") {
                    ...
                    if (softwareSystem.perspectives) {
                        softwareSystem.perspectives.forEach(
                            (perspective) => {
                                if (perspective.name === currentPerspective) {
                                    perspectiveDetails = perspective;
                                }
            ...
            if (perspectiveDetails !== undefined) {
                ...
                perspectiveDescription = structurizr.util
                    .escapeHtml(perspectiveDescription)
                    .replaceAll("\n", "<br />");

                additionalContent +=
                    "<hr" + ' style="border-color:' + style.stroke + '" />';
                additionalContent += "<p><b>Perspective: ";
                additionalContent += structurizr.util.escapeHtml(
                    perspectiveDetails.name,
                );
    ...
    function showTooltipForRelationship(
        relationship,
        relationshipInView,
        style,
        x,
        ...
        if (
            currentPerspective !== undefined &&
            relationshipHasPerspective(relationship) === false
        ) {
            return;
        }
        ...
        if (currentPerspective !== undefined) {
            ...
            if (perspectiveDetails === undefined) {
                if (relationship.linkedRelationshipId) {
                    var linkedRelationship =
                        structurizr.workspace.findRelationshipById(
                            relationship.linkedRelationshipId,
                        );
                    if (linkedRelationship && linkedRelationship.perspectives) {
                        linkedRelationship.perspectives.forEach(
                            (perspective) => {
                                if (perspective.name === currentPerspective) {
                                    perspectiveDetails = perspective;
                                }
            ...
            if (perspectiveDetails !== undefined) {
                ...
                additionalContent +=
                    "<hr" + ' style="border-color:' + style.color + '" />';
                additionalContent += "<p><b>Perspective: ";
                additionalContent += structurizr.util.escapeHtml(
                    perspectiveDetails.name,
                );
        ...
        tooltip.showTooltipForRelationship(
            relationship,
            relationshipInView,
            style,
            x,
    ...
    this.showDiagramScope = function (bool) {
        ...
        if (bool) {
            ...
            if (currentView.type === "SystemContext") {
                ...
                currentView.type === "Dynamic" &&
                currentView.elementId !== undefined
            ) {
                currentView.elements.forEach((elementInView) => {
                    var element = structurizr.workspace.findElementById(
                        elementInView.id,
        ...
        if (self.areKeyboardShortcutsEnabled()) {
            ...
            if (e.which === leftArrow || e.which === pageUp) {
                ...
                if (self.hasElementsSelected() && self.isEditable()) {
                    ...
                    Math.round(canvas.innerWidth()) >
                    Math.round(viewport.innerWidth())
                ) {
                    viewport.scrollLeft(
                        viewport.scrollLeft() - viewport.innerWidth() / 10,
                    );
                    e.preventDefault();
                    return;
                }
                ...
                if (self.hasLinkHighlighted() && self.isEditable()) {
                    ...
                    Math.round(canvas.innerHeight()) >
                    Math.round(viewport.innerHeight())
                ) {
                    viewport.scrollTop(
                        viewport.scrollTop() - viewport.innerHeight() / 10,
                    );
                    e.preventDefault();
                    return;
                }
...
structurizr.ui.PaperSizes = function () {
    ...
    this.getDimensions = (paperSize) => definitions[paperSize];
