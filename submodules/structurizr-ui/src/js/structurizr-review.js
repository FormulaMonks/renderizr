structurizr.Review = function (json, reviewer, canComment) {
    ...
    if (canComment === true) {
        ...
        if (review.type && review.type === "Risk") {
            var html =
                '<button class="btn commentTypeButton riskLowButton" style="background: ' +
                commentTypeColors["RiskLow"] +
                '; color: #ffffff" title="Risk - Low"><img src="/static/img/review/comment-RiskLow.svg" class="icon-btn"/></button>';
            commentTypeButtons.append(html);
            floatingControls.append(html);
            commentModalButtons.append(html);

            html =
                '<button class="btn commentTypeButton riskMediumButton" style="background: ' +
                commentTypeColors["RiskMedium"] +
                '; color: #ffffff" title="Risk - Medium"><img src="/static/img/review/comment-RiskMedium.svg" class="icon-btn"/></button>';
            commentTypeButtons.append(html);
            floatingControls.append(html);
            commentModalButtons.append(html);

            html =
                '<button class="btn commentTypeButton riskHighButton" style="background: ' +
                commentTypeColors["RiskHigh"] +
                '; color: #ffffff" title="Risk - High"><img src="/static/img/review/comment-RiskHigh.svg" class="icon-btn"/></button>';
            commentTypeButtons.append(html);
            floatingControls.append(html);
            commentModalButtons.append(html);
            ...
            var html =
                '<button class="btn commentTypeButton strideButton strideSpoofingButton" style="background: ' +
                commentTypeColors["STRIDE_Spoofing"] +
                '; color: #000000" title="Spoofing"><img src="/static/img/review/comment-STRIDE_Spoofing.svg" class="icon-btn"/></button>';
            commentTypeButtons.append(html);
            floatingControls.append(html);
            commentModalButtons.append(html);
            ...
            html =
                '<button class="btn commentTypeButton strideButton strideTamperingButton" style="background: ' +
                commentTypeColors["STRIDE_Tampering"] +
                '; color: #000000" title="Tampering"><img src="/static/img/review/comment-STRIDE_Tampering.svg" class="icon-btn"/></button>';
            commentTypeButtons.append(html);
            floatingControls.append(html);
            commentModalButtons.append(html);
            ...
            html =
                '<button class="btn commentTypeButton strideButton strideRepudiationButton" style="background: ' +
                commentTypeColors["STRIDE_Repudiation"] +
                '; color: #000000" title="Repudiation"><img src="/static/img/review/comment-STRIDE_Repudiation.svg" class="icon-btn"/></button>';
            commentTypeButtons.append(html);
            floatingControls.append(html);
            commentModalButtons.append(html);
            ...
            html =
                '<button class="btn commentTypeButton strideButton strideInformationDisclosureButton" style="background: ' +
                commentTypeColors["STRIDE_InformationDisclosure"] +
                '; color: #000000" title="Information Disclosure"><img src="/static/img/review/comment-STRIDE_InformationDisclosure.svg" class="icon-btn"/></button>';
            commentTypeButtons.append(html);
            floatingControls.append(html);
            commentModalButtons.append(html);
            ...
            $(".strideInformationDisclosureButton").click(() => {
                this.changeCommentType(
                    "strideInformationDisclosureButton",
            ...
            html =
                '<button class="btn commentTypeButton strideButton strideDenialOfServiceButton" style="background: ' +
                commentTypeColors["STRIDE_DenialOfService"] +
                '; color: #000000" title="Denial of Service"><img src="/static/img/review/comment-STRIDE_DenialOfService.svg" class="icon-btn"/></button>';
            commentTypeButtons.append(html);
            floatingControls.append(html);
            commentModalButtons.append(html);
            ...
            html =
                '<button class="btn commentTypeButton strideButton strideElevationOfPrivilegeButton" style="background: ' +
                commentTypeColors["STRIDE_ElevationOfPrivilege"] +
                '; color: #000000" title="Elevation of Privilege"><img src="/static/img/review/comment-STRIDE_ElevationOfPrivilege.svg" class="icon-btn"/></button>';
            commentTypeButtons.append(html);
            floatingControls.append(html);
            commentModalButtons.append(html);
            ...
            $(".strideElevationOfPrivilegeButton").click(() => {
                this.changeCommentType(
                    "strideElevationOfPrivilegeButton",
            ...
            var count = 0;
            generalColors.forEach((color) => {
                var html =
                    '<button class="commentColor' +
                    count +
                    ' btn commentTypeButton" style="background: ' +
                    color +
                    '"><img src="/static/img/review/comment-General.svg" class="icon-btn"/></button>';
                commentTypeButtons.append(html);
                floatingControls.append(html);
                commentModalButtons.append(html);
    ...
    if (review !== undefined) {
        review.diagrams.forEach((diagram) => {
            renderDiagram(diagram.id);

            if (diagram.comments !== undefined) {
                diagram.comments.forEach((comment) => {
    ...
    $("#commentModal").on("shown.bs.modal", (e) => {
    ...
    $("#commentDescription").on("keypress", (e) => {
        if (e.keyCode === 13) {
            e.preventDefault();
            closeCommentModal();
        }
    ...
    this.changeCommentType = (selector, type, color) => {
        commentType = type;
        commentColor = color;
        ...
        reader.onload = (evt) => {
            try {
                ...
                if (
                    ...
                    fileContent.startsWith("data:image/gif;base64,")
                ) {
    ...
    function renderDiagram(diagramId) {
        ...
        html +=
            '<div class="diagramTitle">Diagram ' +
            diagram.id +
            (canComment === true ? " of " + review.diagrams.length : "") +
            "</div>";
        html +=
            '  <div id="diagram' +
            diagramId +
            'Container" class="diagramContainer img-thumbnail" style="position: relative">';
        html +=
            '    <div id="diagram' +
            diagramId +
            'Viewport" class="diagramViewport">';
        html +=
            '      <div id="diagram' +
            diagramId +
            'Wrapper" class="diagramWrapper">';
        ...
        html +=
            '<button id="diagram' +
            diagramId +
            'ZoomOutButton" class="btn btn-default" title="Zoom out"><img src="/static/bootstrap-icons/zoom-out.svg" class="icon-btn" /></button>';
        html +=
            '<button id="diagram' +
            diagramId +
            'ZoomInButton" class="btn btn-default" title="Zoom in"><img src="/static/bootstrap-icons/zoom-in.svg" class="icon-btn" /></button>';
        html += "</div>";
        ...
        document.getElementById(
            "diagram" + diagramId + "ZoomInButton",
        ).onclick = () => {
            zoomIn(diagramId);
        };

        document.getElementById(
            "diagram" + diagramId + "ZoomOutButton",
        ).onclick = () => {
            zoomOut(diagramId);
        };

        document.getElementById("diagram" + diagramId + "Image").onmouseenter =
            (e) => {
        ...
        var diagramImage = document.getElementById(
            "diagram" + diagramId + "Image",
        );
    ...
    function zoomTo(diagram, scale) {
        ...
        var diagramWrapper = document.getElementById(
            "diagram" + diagramId + "Wrapper",
        );
    ...
    function scaleComments(diagramId) {
        ...
        if (diagram.comments) {
            ...
            allComments.css(
                "background-size",
                scaledWidthAndHeight -
                    scaledPadding +
                    "px " +
                    (scaledWidthAndHeight - scaledPadding) +
                    "px",
            );
    ...
    function renderComment(comment) {
        ...
        document.getElementById(domId).onclick = (e) => {
            if (dragging === false) {
                openCommentModal(comment.id);
            }
        };
        document.getElementById(domId).onmouseover = (e) => {
            if (dragging === false) {
                showTooltip(e, comment.id);
            }
        };
        document.getElementById(domId).onmouseout = (e) => {
            hideTooltip();
        };
    }
    ...
    function deleteComment(id) {
        ...
        if (comment !== undefined) {
            ...
            reviewSession.comments.splice(
                reviewSession.comments.indexOf(comment),
                1,
            );

            review.diagrams.forEach((diagram) => {
                if (diagram.comments.indexOf(comment) > -1) {
                    diagram.comments.splice(
                        diagram.comments.indexOf(comment),
                        1,
                    );
                }
            });
    ...
    this.isUnsavedChanges = () => unsavedChanges;
};
