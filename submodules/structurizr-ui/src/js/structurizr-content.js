...
structurizr.ui.ContentRenderer = function (
    workspace,
    host,
    urlPrefix,
    urlSuffix,
    ...
    md.renderer.rules.code_inline = (tokens, idx, options, env, self) => {
        ...
        if (code.startsWith("$") && code.endsWith("$")) {
            ...
            return default_inline_code_block_renderer(
                tokens,
                idx,
                options,
                env,
    ...
    var default_fenced_code_block_renderer = md.renderer.rules.fence.bind(
        md.renderer.rules,
    );
    md.renderer.rules.fence = (tokens, idx, options, env, self) => {
        ...
        if (token.info === "katex") {
            ...
            code.split(/(?:\n\s*){2,}/).forEach((line) => {
                ...
                try {
                    content += katex.renderToString(line.trim(), {
                        displayMode: true,
                    });
                } catch (err) {
                    content += "<pre>${err}</pre>";
            ...
            return default_fenced_code_block_renderer(
                tokens,
                idx,
                options,
                env,
    ...
    function renderEmbeddedDiagram(diagramIdentifier) {
        ...
        if (view) {
            ...
            var embedUrl =
                ...
                workspace.id +
                "&view=" +
                encodeURIComponent(diagramIdentifier) +
                "&perspective=" +
                ...
                urlPrefix +
                "&urlSuffix=" +
                encodeURIComponent(urlSuffix);

            return (
                '<div style="text-align: center"><iframe id="' +
                id +
                '" class="structurizrEmbed thumbnail" src="' +
                embedUrl +
                '" width="100%" height="' +
                window.innerHeight * MAX_HEIGHT_PERCENTAGE +
                'px" marginwidth="0" marginheight="0" frameborder="0" scrolling="no" allowfullscreen="true"></iframe>' +
                "</div>"
            );
        } else {
            return (
                '<div class="alert alert-danger" role="alert">Unable to embed view \'' +
                diagramIdentifier +
                "' - there is no view with this key in the workspace.</div>"
            );
        }
    }
    ...
    function renderImage(name, alt) {
        ...
        var regex = new RegExp(
            host + "/workspace/" + workspace.id + "/diagrams/(.*?).png",
        );
        ...
        if (image) {
            return (
                '<div style="text-align: center"><img src="data:' +
                image.type +
                ";base64," +
                image.content +
                '" alt="' +
    ...
    this.render = (section) => {
        ...
        if (section.format && section.format === "AsciiDoc") {
            ...
            for (var i = 0; i < lines.length; i++) {
                ...
                if (
                    safeMode &&
                    (line.trim() === "[pass]" || line.trim() === "++++")
                ) {
                    // skip passthroughs
                } else if (safeMode && line.match(inlinePassthroughRegex)) {
                    preparsedContent += line.replaceAll(
                        inlinePassthroughRegex,
                        "[",
                    );
                } else if (line.startsWith("image::embed:")) {
                    var altTagStartIndex = line.indexOf("[");
                    var diagramIdentifier = line.substring(
                        "image::embed:".length,
                        altTagStartIndex,
                    );
                    preparsedContent +=
                        "pass:[" +
                        renderEmbeddedDiagram(diagramIdentifier) +
                        "]";
