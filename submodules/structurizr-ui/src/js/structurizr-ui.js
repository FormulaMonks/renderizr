...
structurizr.ui.applyBranding = () => {
    ...
    var fontNames = "";
    branding.font.name.split(",").forEach((fontName) => {
        fontNames += '"' + structurizr.util.escapeHtml(fontName.trim()) + '", ';
    });

    const brandingStyles = $("#brandingStyles");
    brandingStyles.append(
        "#documentationPanel { font-family: " +
            fontNames.substr(0, fontNames.length - 2) +
            " }",
    );
...
structurizr.ui.loadThemes = (callback) => {
    structurizr.workspace.views.configuration.themes.forEach((theme) => {
...
structurizr.ui.waitForThemesToLoad = (callback) => {
    if (
        structurizr.ui.themes.length <
        structurizr.workspace.views.configuration.themes.length
    ) {
        setTimeout(() => {
            structurizr.ui.waitForThemesToLoad(callback);
        }, 100);
...
structurizr.ui.ElementStyle = function (
    width,
    height,
    background,
    color,
...
structurizr.ui.RelationshipStyle = function (
    thickness,
    color,
    dashed,
    routing,
...
structurizr.ui.findElementStyle = (element, darkMode) => {
    const defaultElementStyle = new structurizr.ui.ElementStyle(
        450,
        300,
        "#dddddd",
        "#000000",
    ...
    const defaultElementStyleForDeploymentNode =
        new structurizr.ui.ElementStyle(
            450,
            300,
            "#ffffff",
            "#000000",
    ...
    const defaultBoundaryStyle = new structurizr.ui.ElementStyle(
        undefined,
        undefined,
        undefined,
        undefined,
    ...
    for (var i = 0; i < tags.length; i++) {
        ...
        if (elementStyle) {
            if (
                elementStyle.width !== undefined ||
                elementStyle.height !== undefined
            ) {
...
structurizr.ui.findRelationshipStyle = (relationship, darkMode) => {
    const defaultRelationshipStyle = new structurizr.ui.RelationshipStyle(
        2,
        "#707070",
        true,
        "Direct",
    ...
    if (structurizr.ui.themes.length > 0) {
        // use the styles defined in the theme as a starting point
        structurizr.ui.themes.forEach((theme) => {
            theme.relationships.forEach((relationshipStyle) => {
                relationshipStylesMap[relationshipStyle.tag] =
                    relationshipStyle;
            });
        });
        ...
        // (1) add workspace styles that are not defined in the theme
        // (2) override the styles defined in the theme where necessary
        structurizr.workspace.views.configuration.styles.relationships.forEach(
            (relationshipStyleFromWorkspace) => {
...
structurizr.ui.getDefaultViewName = (view) => {
    if (view.type === structurizr.constants.FILTERED_VIEW_TYPE) {
    ...
    if (view.type === structurizr.constants.CUSTOM_VIEW_TYPE) {
        return (
            "[Custom] " +
            (view.title && view.title.trim().length > 0
                ? view.title
                : "Untitled")
        );
        ...
        const softwareSystem = structurizr.workspace.findElementById(
        ...
        const container = structurizr.workspace.findElementById(
            view.containerId,
        );
        const softwareSystem = structurizr.workspace.findElementById(
            container.parentId,
        ...
        if (element) {
            if (
                ...
                return (
                    "[Dynamic] " + softwareSystem.name + " - " + element.name
                );
            }
        } else {
            return "[Dynamic]";
        ...
        if (view.softwareSystemId) {
            ...
            return (
                "[Deployment] " + softwareSystem.name + " - " + view.environment
            );
        } else {
...
structurizr.ui.getMetadataForElement = (element, includeTechnology) => {
    const openingSymbol =
        structurizr.ui.openingMetadataSymbols[
            structurizr.workspace.views.configuration.metadataSymbols
        ];
    const closingSymbol =
        structurizr.ui.closingMetadataSymbols[
            structurizr.workspace.views.configuration.metadataSymbols
    ...
    if (element.type === structurizr.constants.CUSTOM_ELEMENT_TYPE) {
        ...
        if (includeTechnology === true && element.technology) {
            return (
                openingSymbol +
                structurizr.workspace.getTerminologyFor(element) +
                ": " +
                element.technology +
                closingSymbol
            );
...
structurizr.ui.getMetadataForRelationship = (relationship) => {
...
structurizr.ui.isFullScreenEnabled = () => (
...
structurizr.ui.isFullScreen = () => (
        document.enterFullScreen ||
