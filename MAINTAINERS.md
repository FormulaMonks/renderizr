...
- [ ] **The automation GitHub App.** *Optional for releases, required to automate dependency updates.* Releases do not depend on it: `pnpm release:pr` and `pnpm release:tag` run the same release-please against your own `gh` login, with nothing stored anywhere — see [How a release happens](CONTRIBUTING.md#how-a-release-happens). Creating an App needs organization access, so treat this as the upgrade you make when someone has it, not a blocker.* One App serves both `release.yml` and `renovate.yml`. Create it under the FormulaMonks organization with repository permissions **Contents: read and write**, **Pull requests: read and write**, **Issues: read and write** (Renovate's dependency dashboard) and **Workflows: write** (Renovate edits the pinned action digests in `.github/workflows`, which no other permission covers), install it on `renderizr` only, then add the App ID as the repository *variable* `AUTOMATION_APP_ID` and the generated private key as the repository *secret* `AUTOMATION_APP_PRIVATE_KEY`. Both workflows skip themselves and say so while the variable is unset, so the Actions tab stays green rather than red in the meantime. Two reasons it is an App rather than a personal access token: a PAT expires and ties the project's automation to one person's account, and an App can be added to the `main` ruleset's bypass list, which is the next item.
- [ ] **Add the App to the `main` ruleset's bypass list.** Only after the App is installed — the bypass picker lists apps installed on this repository, which is why searching it for "Renovate" today returns nothing. Settings → Rules → `main` → Bypass list → Add bypass → Apps → the automation App, mode "Always". Without it, Renovate's pull requests sit forever with a green `ci` check: the ruleset requires one approving review and a code-owner review, and no amount of automation can satisfy a review requirement. Understand the trade before making it. With the bypass, dependency updates that pass the full suite and both render modes merge without a human reading them; the same App also merges release pull requests. Three things bound the blast radius, and all three are load-bearing rather than decorative once the bypass exists: `automerge` is `false` at the top of `renovate.json` so nothing merges unless a rule names it, `minimumReleaseAge: "3 days"` means nothing merges the day it is published, and the last rule in `packageRules` forbids auto-merge for every package inlined into rendered output — jquery, `@joint/**`, `@dagrejs/**`, markdown-it, highlight.js, history and vite. If that trade is not one you want, skip this item: Renovate still opens its pull requests and you still review them, which is what happens today.

- [ ] **The release GitHub App.** *Required before any release can happen.* Releases are dead in the water without it: `gh api repos/FormulaMonks/renderizr/actions/permissions/workflow` reports `can_approve_pull_request_reviews: false`, so `GITHUB_TOKEN` cannot open the release pull request and release-please fails with "GitHub Actions is not permitted to create or approve pull requests". Ticking Settings → Actions → General → "Allow GitHub Actions to create and approve pull requests" would clear that error and immediately create a worse one: a pull request opened with `GITHUB_TOKEN` does not trigger workflows, so `ci` would never report on the release pull request, and `ci` is a required check in the ruleset above — every release would need a ruleset bypass. An App identity avoids both. Create a GitHub App owned by the FormulaMonks organization with repository permissions **Contents: read and write** and **Pull requests: read and write**, install it on this repository only, then add the App ID as the repository *variable* `AUTOMATION_APP_ID` and its generated private key as the repository *secret* `AUTOMATION_APP_PRIVATE_KEY`. Until `AUTOMATION_APP_ID` is set, `.github/workflows/release.yml` skips release-please and runs a `prepare release (not configured)` job that says so — the run is green, and nothing is released.
