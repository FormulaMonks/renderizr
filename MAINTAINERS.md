...
- **Ninety-eight accounts with `read`**, likewise inherited from the organisation.
...
`main` is not branch-protected today — `gh api repos/FormulaMonks/renderizr/branches/main/protection` returns 404 "Branch not protected" and `gh api repos/FormulaMonks/renderizr/rulesets` returns `[]`. So what follows is a promise rather than a setting, and it binds every account that can push, including the inherited-admin ones listed above. Hold us to it in public if we break it.
...
- [ ] **Branch protection or a ruleset on `main`.** Not configured: `gh api repos/FormulaMonks/renderizr/branches/main/protection` returns 404 "Branch not protected" and `gh api repos/FormulaMonks/renderizr/rulesets` returns `[]`. Until it is, the rules in [the section above](#the-rules-maintainers-hold-themselves-to) are a promise. The settings that would match them: require a pull request, require one approving review, require the `ci` checks, and no bypass for administrators.
...
- [ ] **`NPM_TOKEN` secret**, if and when the package is published to npm. `.github/workflows/release.yml` reads `secrets.NPM_TOKEN` in its manually dispatched publish job; without it that job is the only part of the release pipeline that cannot run.
